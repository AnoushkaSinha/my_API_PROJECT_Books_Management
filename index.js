...
booky.get("/is/:isbn", (req, res) => {
    ...
    if (getSpecifiedBook.length === 0) {
        return res.json({ error: `No such book found with this ${req.params.isbn} ` });
    }
...
booky.get("/c/:category", (req, res) => {
    ...
    if (getSpecifiedBook.length === 0) {
        return res.json({error:`No such book found with ${req.params.category}category`})
    }
